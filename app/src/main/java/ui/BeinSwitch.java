...
public class BeinSwitch extends ViewGroup {
    ...
    public BeinSwitch(Context context) {
        super(context);
        init(null);
    }

    public BeinSwitch(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public BeinSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public BeinSwitch(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attr) {
        ...
        if (attr != null) {
            TypedArray ta = getContext().obtainStyledAttributes(attr, R.styleable.BeinSwitch);
            iconSize = ta.getDimensionPixelSize(R.styleable.BeinSwitch_isw_icon_size, iconSize);
            leftIcon.setImageDrawable(ta.getDrawable(R.styleable.BeinSwitch_isw_icon_left));
            rightIcon.setImageDrawable(ta.getDrawable(R.styleable.BeinSwitch_isw_icon_right));
            inactiveTintIconLeft = ta.getColor(R.styleable.BeinSwitch_isw_inactive_tint_icon_left, colorDefInactive);
            activeTintIconLeft = ta.getColor(R.styleable.BeinSwitch_isw_active_tint_icon_left, colorDefActive);
            inactiveTintIconRight = ta.getColor(R.styleable.BeinSwitch_isw_inactive_tint_icon_right, colorDefInactive);
            activeTintIconRight = ta.getColor(R.styleable.BeinSwitch_isw_active_tint_icon_right, colorDefActive);
            background.setColor(ta.getColor(R.styleable.BeinSwitch_isw_background_color, colorDefBackground));
            thumbColorLeft = ta.getColor(R.styleable.BeinSwitch_isw_thumb_color_left, colorDefThumb);
            thumbColorRight = ta.getColor(R.styleable.BeinSwitch_isw_thumb_color_right, colorDefThumb);
            currentChecked = Checked.values()[ta.getInt(R.styleable.BeinSwitch_isw_default_selection, 0)];
            ta.recycle();
        } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int overshootPadding = Math.round(thumbDiameter * 0.05f);
